...
def ml_prediction(x_train, x_test, y_train, y_test):
    ...
    time_train_ml.append(total_time/len(y_train))

    start_time = time.time()
    y_pred = clf.predict(x_test)
    total_time = time.time() - start_time
    time_predict_ml.append(total_time)
...
def to_message_lst(msg_obj):
    msg_seg = crf.crfpp(msg_obj.message)
    msg_data = ' '.join(msg_seg)
    return msg_data

def topic_detection(x_train, x_test, y_train, y_test):
    ...
    total_time = time.time() - start_time
    time_train_topic.append(total_time/len(y_train_inner))

    x_test_corpus = []
    per_y_pred = []
    start_time = time.time()
    for x_data in x_test:
        data_seg = crf.crfpp(x_data.message)
        data = ' '.join(data_seg)
        x_test_corpus.append(data)

    for x in x_test:
        test_message = to_message_lst(x)
    ...
    total_time = time.time()-start_time
    time_predict_topic.append(total_time/len(y_test))
...
def topic_prediction(x_train, x_test, y_train, y_test):
    ...
    total_time = time.time()-start_time
    time_train_text.append(total_time/len(y_train))

    start_time = time.time()
    y_pred = text_clf.predict(x_test_msg)
    total_time = time.time()-start_time
    time_predict_text.append(total_time/len(y_pred))
